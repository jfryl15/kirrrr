...
# Python version must be declared BEFORE any FROM that uses it.
...
# ---------- Runtime ----------
# Re-declare the ARG because ARG scope does not automatically continue
...
RUN chmod +x \
    /usr/bin/hpxpanel-cli \
    /usr/bin/hpxpanel-tui \
    /code/healthcheck.sh \
    /code/start.sh \
    /code/scripts/hpx-pulse-agent.sh \
    /code/scripts/hpx-tunnel-engine-install.sh \
    2>/dev/null || true

# The tunnel-engine release is optional for Railway.
ARG TARGETARCH
...
RUN mkdir -p /code/bundled/tunnel-engine \
    ...
    && if curl -fsSL \
         "https://github.com/pooyahpx/HPXPANEL/releases/download/${ENGINE_TAG}/${ENGINE_ASSET}" \
    ...
    && curl -fsSL \
         "https://github.com/pooyahpx/HPXPANEL/releases/download/${ENGINE_TAG}/SHA256SUMS" \
         -o "/code/bundled/tunnel-engine/SHA256SUMS" \
         || true
